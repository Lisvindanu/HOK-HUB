...
function extractPassiveName(description: string): string | null {
  const m = description.match(/^Passive\s*-\s*([^\n\r]+)/);
...
function EmptySlot() {
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className="text-[9px] text-white/10 text-center w-12">Empty</span>
    </div>
  );
}
                    ...
                    <div key={name} className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-red-400/70 mt-0.5">
                          {items.map(i => i.name).join(' + ')} — keduanya punya passive yang sama, hanya satu yang aktif.
                        </p>
                      </div>
                    </div>
                  ))}
