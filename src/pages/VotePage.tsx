...
const RECENTLY_IMPLEMENTED = [
  { label: 'Dropdown "More" — text nowrap fix', date: 'Mar 2026' },
  { label: 'Draft Pick — Switch Side per game', date: 'Mar 2026' },
];
...
export function VotePage() {
  ...
  return (
    <div className="min-h-screen bg-dark-400">
      ...
      {/* Recently Implemented Ticker */}
      {RECENTLY_IMPLEMENTED.length > 0 && (
        <div className="border-y border-green-500/[0.15] bg-green-500/[0.04] py-2 overflow-hidden">
          <div className="flex items-center">
            <div className="flex-shrink-0 flex items-center gap-2 px-4 bg-dark-400/90 backdrop-blur-sm border-r border-green-500/[0.15] pr-4 z-10">
              <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
              <span className="text-xs font-semibold text-green-400 whitespace-nowrap uppercase tracking-wider">
                Baru Diimplementasi
              </span>
            </div>
            <div className="overflow-hidden flex-1">
              ...
              >
                {[...RECENTLY_IMPLEMENTED, ...RECENTLY_IMPLEMENTED].map((item, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center gap-2 text-xs text-gray-400 flex-shrink-0 px-6"
                  >
                    <CheckCircle2 className="w-3.5 h-3.5 text-green-400 flex-shrink-0" />
                    <span>{item.label}</span>
                    <span className="text-green-500/50 font-medium">{item.date}</span>
